...
std::vector<PerpRow> computeRanking(long long windowSec, bool& ok) {
    ...
    while ((stepRc = sqlite3_step(s)) == SQLITE_ROW) {
        ...
        ser.pnl += pnl;
        if (margin > 0) ser.margin = margin;
        else if (lev > 0 && notional > 0) ser.margin = notional / lev;
    ...
    for (auto& kv : accs) {
        ...
        if (a.margin > 0) {
            r.roiPercent = 100.0 * static_cast<double>(a.roiPnl) / static_cast<double>(a.margin);
            r.roiKnown = true;
        }
