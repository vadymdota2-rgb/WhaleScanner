...
const std::unordered_map<std::string, Entry>& table() {
    static const std::unordered_map<std::string, Entry> t = {
        ...
        {"hold_illiquid", {"Marked positions are larger than their pool can absorb - the real sale price would be much lower.",
                           "Отмеченные позиции крупнее своего пула — продать их по рыночной цене не выйдет, реальная выручка будет заметно ниже."}},
