...
const std::unordered_map<std::string, Entry>& table() {
    static const std::unordered_map<std::string, Entry> t = {
        ...
        {"menu_hold", {"🔍 BSC Spot Portfolio", "🔍 Портфель BSC Спот"}},
        {"hold_title", {"🔍 <b>BSC Spot Portfolio</b>", "🔍 <b>Портфель BSC Спот</b>"}},
        {"hold_prompt", {"🔍 <b>BSC Spot Portfolio</b>\n\nSend a wallet address — 0x..., 42 characters — and I will show what it holds right now.\n\nAny address works, not only the ones you track.",
                         "🔍 <b>Портфель BSC Спот</b>\n\nПришлите адрес кошелька — 0x..., 42 символа — и я покажу, что в нём сейчас.\n\nПодойдёт любой адрес, не только из отслеживаемых."}},
        {"hold_bad_address", {"❌ Invalid wallet address.\n\nThe address must start with 0x and be 42 characters long. Send a valid address or tap Cancel.",
                              "❌ Неверный адрес кошелька.\n\nАдрес должен начинаться с 0x и содержать 42 символа. Пришлите верный адрес или нажмите Отмена."}},
        {"hold_loading", {"⏳ Reading balances from the blockchain, this takes a few seconds...",
                          "⏳ Читаю балансы из блокчейна, это займёт несколько секунд..."}},
        {"hold_total", {"Total value", "Общая стоимость"}},
        ...
        {"hold_illiquid", {"Marked positions are larger than their pool — the market price shown is not achievable on sale.",
                           "Отмеченные позиции крупнее своего пула — по указанной цене их не продать, выручка будет ниже."}},
        ...
        {"ton_memo_warning", {"⚠️ Without the comment the payment cannot be matched to your account.",
                              "⚠️ Без комментария платёж не получится связать с вашим аккаунтом."}},
        ...
        {"ton_rate_unavailable", {"⚠️ Cannot get the GRAM rate right now, so the amount cannot be calculated.\n\nTry again in a minute or pay with Stars.",
                                  "⚠️ Не удаётся получить курс GRAM, поэтому сумму не посчитать.\n\nПопробуйте через минуту или оплатите звёздами."}},
