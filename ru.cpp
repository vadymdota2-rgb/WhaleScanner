...
const std::unordered_map<std::string, Entry>& table() {
    static const std::unordered_map<std::string, Entry> t = {
        ...
        {"ai_st_quality", {"Model quality", "Качество модели"}},
        ...
        {"ai_st_loss", {"Estimate error", "Ошибка оценок"}},
        {"ai_st_wf", {"Holds up across periods", "Держится на разных отрезках"}},
        ...
        {"ai_auc_hint", {"0.50 is a coin flip, 1.00 is flawless", "0.50 — это как монетка, 1.00 — без ошибок"}},
        {"ai_loss_hint", {"next to it, the same error without the model: lower means more accurate", "рядом — та же ошибка без модели: чем меньше, тем точнее"}},
        {"ai_wf_hint", {"checked on several stretches of time in a row", "проверяем на нескольких отрезках времени подряд"}},
